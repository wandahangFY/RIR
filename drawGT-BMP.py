...
if __name__=="__main__":
    src_dir= r"D:\wan\BaiduNetdiskDownload\Train_v3\image_SBS-2022"
    save_dir=r"D:\wan\BaiduNetdiskDownload\Train_v3\image_SBS-2022-GT"
    ...
    for txt_path in dir_list:
        ...
        # 读取图像文件
        img = cv2.imread(src_dir+"/"+str(txt_path.replace('.txt','.bmp')))
        h, w = img.shape[:2]
        annotator = Annotator(img, line_width=20, example=str(names))
        for x in lb:
        ...
        save_path=save_dir+"/"+txt_path.replace('.txt','.bmp')
        print(save_path)
