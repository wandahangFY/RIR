...
def create_dataloader(path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
                      rect=False, rank=-1, workers=8, image_weights=False, quad=False, prefix='', shuffle=False,val_flag=False,rir_flag=False,
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',val_flag=False,rir_flag=False,
                 ):
        self.img_size = img_size
        ...
        try:
            self.cv_resize_flags_with_weights[cv2.INTER_NEAREST]=hyp['rir_INTER_NEAREST']
            self.cv_resize_flags_with_weights[cv2.INTER_LINEAR] = hyp['rir_INTER_LINEAR']
            self.cv_resize_flags_with_weights[cv2.INTER_CUBIC] = hyp['rir_INTER_CUBIC']
            self.cv_resize_flags_with_weights[cv2.INTER_AREA] = hyp['rir_INTER_AREA']
            self.cv_resize_flags_with_weights[cv2.INTER_LANCZOS4] = hyp['rir_INTER_LANCZOS4']
            self.cv_resize_flags_with_weights[cv2.INTER_LINEAR_EXACT] = hyp['rir_INTER_LINEAR_EXACT']

        except Exception as e:
            print(f'rir_INTER_NEAREST not set in hyp, use default rir settings: {e}\n')
    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            ...
            if npy and npy.exists():  # load npy
                ...
                # im =OneToThreeGaussianBlur(im)
                # im = receptiveField(im)
                # im = OneToThree2_ORDER_RB(im)
                # im =OneToThree_CLAHE_USM_P(im)
            ...
            if r != 1:  # if sizes are not equal
                ...
                if self.rir_flag:  #   use random_interpolation_resize
                    if self.val_flag:   #   validation
                        im = cv2.resize(im,
                                        (int(w0 * r), int(h0 * r)),
                                        interpolation=cv2.INTER_LINEAR if (self.augment or r > 1) else cv2.INTER_AREA)
                    else:
                        # print(self.cv_resize_flags_with_weights)
                        im = cv2.resize(im,
                                        (int(w0 * r), int(h0 * r)),
                                        interpolation=random_interpolation_resize(cv_resize_flags_with_weights=self.cv_resize_flags_with_weights))
                else:
                    im = cv2.resize(im,
                                    (int(w0 * r), int(h0 * r)),
                                    interpolation=cv2.INTER_LINEAR if (self.augment or r > 1) else cv2.INTER_AREA)
...
if __name__=="__main__":
    print(random_interpolation_resize())