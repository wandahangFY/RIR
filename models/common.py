...
# ---------------------------- WeightConcat ---------------------------------
class WeightConcat(nn.Module):
    # Concatenate a list of tensors along dimension
    def __init__(self, c1, c2):
        super(WeightConcat, self).__init__()
        # 设置可学习参数 学习不同分支的权重
    ...
    def forward(self, x):  # mutil-layer 1-3 layers #ADD or Concat
        ...
        if len(x) == 2:  # 两个分支add操作
            ...
            x = self.conv(self.act(0.5 * x[0] + 0.5 * x[1]))
            ...
            x = self.conv(self.act(0.33 * x[0] + 0.33 * x[1] + 0.33 * x[2]))
        return x
...
class AdaptiveConcat(nn.Module):
    # Concatenate a list of tensors along dimension
    def __init__(self, c1, c2):
        super(AdaptiveConcat, self).__init__()
        # 设置可学习参数 学习不同分支的权重
        # 设置可学习参数 nn.Parameter的作用是：将一个不可训练的类型Tensor转换成可以训练的类型parameter
        # 并且会向宿主模型注册该参数 成为其一部分，
        # print(c1,c2)
        ...
        # print(c1)
        if len(c1)==2:
            self.conv0 = Conv(c1[0], c2, 1, 1, 0)
            self.conv1 = Conv(c1[1], c2, 1, 1, 0)
        elif len(c1)==3:
            self.conv0 = Conv(c1[0], c2, 1, 1, 0)
            self.conv1 = Conv(c1[1], c2, 1, 1, 0)
            self.conv2 = Conv(c1[2], c2, 1, 1, 0)
        ...
        # for i in c1:
        #     # print(i)
        #     self.conv_list.append(Conv(i, c2, 1, 1, 0))
    ...
    def forward(self, x):  # mutil-layer 1-3 layers #ADD or Concat
        ...
        if len(x) == 2:  # 两个分支add操作
            ...
            weight = w / (torch.sum(w, dim=0) + self.epsilon)
            # Fast normalized fusion
            x = self.conv(self.act(weight[0] * self.conv0(x[0]) + weight[1] * self.conv1(x[1])))
        elif len(x) == 3:  # 三个分支add操作
            w = self.w2_weight
            # 将权重进行归一化
            weight = w / (torch.sum(w, dim=0) + self.epsilon)
            # Fast normalized fusion
            x = self.conv(self.act(weight[0] * self.conv0(x[0]) + weight[1] * self.conv1(x[1]) + weight[2] * self.conv2(x[2])))
        return x
...
# ---------------------------- C3CBAM ---------------------------------
class CA2Bottleneck(nn.Module):
    # ch_in, ch_out, shortcut, groups, expansion, ratio, kernel_size
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5, ratio=16, kernel_size=7):
        super(CA2Bottleneck, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)
        self.add = shortcut and c1 == c2
        ...
        self.spatial_attention = CA(c1,c2,ratio)
...
class UPConv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.up = nn.Sequential(
            ...
            Conv(in_ch, out_ch, k=3, s=1, p=1, g=1, act=True),
        )
...
class AdaNode(nn.Module):
    """
    Up Convolution Block
    """

    def __init__(self, in_up_ch,in_ch, out_ch):
        super(AdaNode, self).__init__()
        self.up = UPConv(in_up_ch, out_ch)
        self.conv = Conv(in_ch, out_ch, k=3, s=1, p=1, g=1, act=True)
        print(in_up_ch,in_ch, out_ch)
...
if __name__=="__main__":
    ...
    node=AdaNode(64,32,128)
    node2 = AdaNode(128, 64, 128)
    a1 = torch.randn(1, 64, 16, 16)
    a2 = torch.randn(1, 32, 32, 32)
    a4 = torch.randn(1, 64, 64, 64)
    a3=node([a1, a2])
    print(a3.shape)
    a5=node2([a3, a4])
    print(a5.shape)
